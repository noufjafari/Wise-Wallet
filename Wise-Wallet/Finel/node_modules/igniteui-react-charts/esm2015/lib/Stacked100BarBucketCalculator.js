        ...
        getBucket1(a, b, c, d, e, f) {
            ...
            if (s <= t) {
                for (let u = s; u <= t; ++u) {
                    ...
                    k = Math.abs(g.tn[u]) + g.tm[u];
                    if (j < l) {
                        p = Math.min(p, (i.logicalSeriesLink.a7._inner[u] + j) / k * 100);
                        o = Math.max(o, i.logicalSeriesLink.a7._inner[u] / k * 100);
                    }
                    else {
                        p = Math.min(p, i.logicalSeriesLink.a6._inner[u] / k * 100);
                        o = Math.max(o, (i.logicalSeriesLink.a6._inner[u] + j) / k * 100);
                    }
                    if (!isNaN_(m)) {
                        if (!isNaN_(p)) {
                            m = Math.min(m, p);
